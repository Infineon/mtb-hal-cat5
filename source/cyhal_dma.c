...
{
    // TODO: Unclear how error conditions need to be processed
    cyhal_dma_event_callback_t callback = (cyhal_dma_event_callback_t)(((cyhal_dma_t *)obj)->callback_data.callback);
    (callback)(((cyhal_dma_t *)obj)->callback_data.callback_arg, CYHAL_DMA_TRANSFER_COMPLETE | CYHAL_DMA_DESCRIPTOR_COMPLETE);
}
...
cy_rslt_t cyhal_dma_configure(cyhal_dma_t *obj, const cyhal_dma_cfg_t *cfg)
{
    ...
    if (status == CY_RSLT_SUCCESS)
    {
        BTSS_DMAC_CONFIG_REG_t cfg_reg = (BTSS_DMAC_CONFIG_REG_t)(obj->dma_req.DMACCxConfiguration);
        ...
        BTSS_DMAC_CONTROL_REG_t ctrl_reg = (BTSS_DMAC_CONTROL_REG_t)(obj->dma_req.DMACCx_first_segment_regs.DMACCxControl);
...
cy_rslt_t cyhal_dma_start_transfer(cyhal_dma_t *obj)
{
    ...
    if (obj->is_enabled)
    {
        ...
        btss_dmac_requestTransfer(&(obj->dma_req));
    }
