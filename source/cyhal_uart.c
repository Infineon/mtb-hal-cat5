...
static bool _cyhal_uart_pm_switch_to_gpio(en_hsiom_sel_t *saved_hsiom, GPIO_PRT_Type *port, uint8_t pin)
{
    bool status = true;
    ...
    if (NULL != port)
    {
        ...
        *saved_hsiom = Cy_GPIO_GetHSIOM(port, pin);
    #if defined(COMPONENT_CAT5)
        /* Set the GPIO to output and to logic 1*/
        status = _cyhal_gpio_switch_and_set(pin);
        if(!status)
        {
            /* Restore saved function */
            Cy_GPIO_SetHSIOM(port, pin, *saved_hsiom);
        }
    #else
...
static bool _cyhal_uart_pm_callback_instance(void *obj_ptr, cyhal_syspm_callback_state_t state, cy_en_syspm_callback_mode_t pdl_mode)
{
    ...
    // We need to explicitly cast the ternary result because on CAT5 devices the ARM compiler
    // treates the NULL within the ternary as an integer rather than a pointer and produces
    // a warning.

    GPIO_PRT_Type *txport = (GPIO_PRT_Type *)(obj->pin_tx != NC ? CYHAL_GET_PORTADDR(obj->pin_tx) : NULL);
    GPIO_PRT_Type *rtsport = (GPIO_PRT_Type *)(((obj->pin_rts != NC) && obj->rts_enabled) ? CYHAL_GET_PORTADDR(obj->pin_rts) : NULL);
   ...
   if (state == CYHAL_SYSPM_CB_SYSTEM_NORMAL || state == CYHAL_SYSPM_CB_SYSTEM_LOW)
    {
        ...
        {
            allow = _cyhal_uart_pm_switch_to_gpio(&(obj->saved_rts_hsiom), rtsport, rtspin);
        }
    }
    else
    {
                    ...
                    */
                    if (Cy_SCB_UART_IsTxComplete(obj->base))
                    {
                        if (0UL == Cy_SCB_UART_GetNumInRxFifo(obj->base))
                        {
                            ...
                            */
                            allow = _cyhal_uart_pm_switch_to_gpio(&(obj->saved_tx_hsiom), txport, txpin);
...
static cy_rslt_t _cyhal_uart_init_hw(cyhal_uart_t *obj)
{
    ...
    {
        // Clock initialization is done by the Device Configurator.
        // Generated code contains Cy_SCB_EnableClock() with desired Clock configuration.
    }
    #endif
